...
async function fillListProducts() {
    ...
    products.forEach(function (item, index) {
        $(".list tbody").append(
            ...
            <td>
                <a href="#" data-toggle="modal" data-target="#addAndUpdateProductModal" onclick="openModalUpdate(${item.id})">Xem/Sửa</a><br>
                <button id="btn-active-pro" onclick="handleClickActive(${item.id})">
    ...
    for (let index = 1; index <= pages; index++) {
        ...
        $(".pagination").append(
            `
                <button class="${index == curPage ? "active" : null}" onclick="handleClick(${index})">
                    ${index}
                </button>
            `
        );  
...
function openAddProductModal() {
    ...
    $.get("http://localhost:8080/api/v1/products/existsTitle/" + title, function (data, status) {
        ...
        if (data) {
            // show error message
            showFieldErrorMessage("modal-input-errMess-title", "modal-title", "Sản phẩm này đã tồn tại!");
            return;
        } else {
        
            createProductViaAPI(title , originalPrice , promotionPrice , cat , amount , des);
        }
    });
}
...
async function createProductViaAPI(title , originalPrice , promotionPrice , cat , amount , des) {
...
function openModalUpdate(id){
    ...
    document.getElementById('modal-originalPrice').value = product[0].originalPrice;
    document.getElementById('modal-promotionPrice').value = product[0].promotionPrice;
    ...
    $('#image_product').append(
        `<img id="image_preview" src="${serverUrl}/${product[0]?.productImages[0]?.imagePublicId}" width="128px" height="128px" alt=""></img>`
    );
